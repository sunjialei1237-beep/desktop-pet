...
async fn proactive_bubble_brings_up_due_pending() {
    ...
    let bubble = proactive::generate(&db, &llm, None, &[], proactive::DEFAULT_MEMORY_RATIO, false)
        .await
...
async fn proactive_bubble_fulfills_pet_promise() {
    ...
    let outcome = proactive::generate(&db, &llm, None, &[], proactive::DEFAULT_MEMORY_RATIO, false)
        .await
