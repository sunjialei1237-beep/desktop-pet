// Poll window.__dragDiag at a given interval for a given duration.
// Usage: node scripts/cdp_dragdiag.cjs [durationMs] [intervalMs]
async function main() {
  const dur = Number(process.argv[2] || 8000), int = Number(process.argv[3] || 300);
  ...
  const rect = await ev(`JSON.stringify((()=>{const r=window.__ctDiag?window.__ctDiag():null;return r})())`);
  console.log("CT " + rect);
  const t0 = Date.now();
