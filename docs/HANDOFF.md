...
## §当前任务（接手者先看这）
> **2026-08-15（续³⁹·2）更新 · OS 顶边钳制根治 ✅（tsc 0 / 完整回归套件实测通过 / release 已 rebuild + 重启）**。用户"上移后依旧恢复原位"终局确认：落体已关后，**把桌宠拖到屏幕高处（窗口顶边出屏）松手，Windows 强制弹回 T=0**——这是"放哪停哪"的最后一块。**根因**：① OS 原生拖拽松手时窗口顶边 T<0 会被强制钳制回 0（实测 2ms 内）；② 弹回的 onMoved 先于 lbutton=false 光标事件到达 → onMoved 式捕获被覆盖为 0，恢复失败。**修法**：**光标式捕获为主**——OS 拖拽保持"光标↔窗口"偏移恒定，`grabOffsetRef`=阈值跨越瞬间 client×scale，松手点=`lastHeldCursor − grabOffset`（免疫时序竞态）；onMoved 式兜底。**三条恢复路径**：onMoved 快路径（弹回 move 上即时恢复）/ 释放快路径（lbutton true→false 后 80ms，实测 ~120ms）/ arm 兜底（300ms 静默校准）。**防误触守卫**：恢复需 `wasDragged`（证明刚发生真实 OS 拖拽，grabOffset 新鲜）+ `snapped.y<SNAP_GAP`（证明窗口真被钳制到顶边）双验证——穿透区"按下+移动"（无真实拖拽）或 startDragging 失败均不再误恢复。**验证**（`scripts/drag_regression.ps1` + `scripts/cdp_console_diag.cjs`，CDP 9223）：T1 出屏拖 600 → 弹回 0 → 一次快路径恢复至松手点 T=-365 稳定停留；T2 穿透按下+光标移动 → 不误恢复；T3 屏内上拖 → 停 135；T4 下拖 → 停 427。**踩坑**：① 回归脚本 `SetWindowPos` 必须带 `SWP_NOSIZE(0x0001)`，否则 cx=cy=0 把桌宠窗口缩成 0×0（曾导致后续所有拖拽测试"失灵"，实为窗口消失）；② 拖拽阈值（5px）使释放点与理论值差 ~7-17px 属正常；③ 测试机 Edge 全屏时会盖住桌宠（z-order 正常但按下落空），回归前确认窗口完整可见。
