...
> **2026-08-15（续³⁷）更新 · 落体手感再调 ✅（commit `5d8040d`，tsc 0 / vitest 34 绿 / release 已 rebuild + 重启）**。用户"下落距离再砍一半，做出缓缓飘落的感觉"：落点弧线 **1/3 → 1/6**（App.tsx fallLimit）+ **GRAVITY 1200/9 → 1200/18**——距离与重力同时减半 ⇒ 下落时长不变、全程速度减半（飘而不拖）。调参入口：`src/animation/gravity.ts` 顶部 `GRAVITY` + `App.tsx` fallLimit 的 `/ 6`。📋 用户实跑感受手感，嫌快/嫌慢动这两个数。
