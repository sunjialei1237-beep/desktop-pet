...
> **2026-08-18（续⁵²·4）· release 环境采样击穿三连修复 ✅（本 commit）**。起因（用户）：找不到 VSCode 根因 + 专注觉得像白名单；以及上一轮三修复（`查看今天要交的报告.md` planner 命中 / U1「可以」续做 / F12 Focus 广义工作计数 `7a9f780`）真机复查时暴露的新层问题——主题「环境根拿到但模型/路径用不对」→「环境根干脆没拿到」。本轮四件套：① **backtrack pid**：pet/debug 自聚焦的 last_sample 回退保留原始 (app,title,**pid**)，编辑器 root 查询不再拿 pet pid 当 Code pid；且 nil 不再入缓存，下一次采样重试。② **拥有式打开优先环境**：`我打开的/打开的文件/我打开的代码` planner 直接 SystemObservation，不吃 `打开` 的 ComputerAction。③ **确定性路径最后一公里**：section root → `set_turn_root` 在同一函数内 pinned；`env_had_root` 与 section 同源；注入 root 则 `tool_choice="required"`；fs 全部路径入参 `path_arg` → `hydrate_relative_path`（绝对路径不碰、`.`/裸文件名拼 root）；DeepSeek 的 `demo.ts` 找不到时用 root 下有界文件名搜索找到真实 `src/demo.ts` 再走授权。④ **Focus 白名单翻转**：默认 Work，明确非工作清单（浏览器/社交/娱乐/系统）才不计。⑤ DebugPanel `路径:` 显示当前采样 root。**验证**：lib 536 / all-targets --no-run 0 / release exe 19:21 重建；真机 `D:\environment-demo` 打开 demo.ts 场景全链路日志（其下）。**接手待办**：25min 锁、User 日常「我打开的××」按 words suggested in guide 验收、写侧 F1/F2 按 guide C 段；HANDOFF 本次已更新，origin==HEAD。
