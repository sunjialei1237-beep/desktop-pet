...
## §当前任务（接手者先看这）
**docs 治理完成（2026-07-26）：去 `superpowers/` 嵌套 + 归档过期文档。** design/plan 提到 `docs/specs/`+`docs/plans/`（去 superpowers 层）；bug-audit/fix-plan/feature-checklist/known-issues 进 `docs/archive/`；同步 8 处路径引用（CLAUDE.md ×5 / Architecture-Principles ×2 / implementation-plan ×1）。纯文档无代码风险，grep 无断链。等选下一步（见 §下一步候选）。

## §最近一轮 (2026-07-26)：docs 治理 — 去 superpowers 嵌套 + 归档过期
