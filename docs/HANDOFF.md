...
> **2026-08-18（续⁵²·3）· release 实装又揪出三缺陷并修复（commit `7a9f780`，origin==HEAD）**。桌签 shortcut release 是用户真实的日常工作副本，用它的聊天日志复测：
> 1. **裸读命令不路由**：`查看今天要交的报告.md` 原 planner 无命中 → capability=None → 模型答"我看不到你的文件"（DB 日志实锤）。补 `查看/看下/读一下/读取/帮我看/帮我读/read this/read the file` + 回归；release 复跑：SystemObservation → 授权问 → `read_text_file Success`。
> 2. **U1「可以」后不保证继续干活**：授权续做轮 tool_choice=auto 时 DeepSeek 偶尔不调工具反而追要路径。修复：`LlmClient::chat_required_tools` + `run_agent_loop(force_first_tool)`，仅授权续做第一轮 `tool_choice="required"`（其余轮 auto 不变）。release 复跑日志：Rejected → ONCE grant → U1 rerun → read Success。
> 3. **F12 Focus 恒 0（用户主诉）**：Notepad/Typora/Obsidian/WPS 等裸文本编辑器原归 Other，聚焦时钟永不累计。已入 Work 分类；release 前台 Notepad 66s 实测 F12 Focus 走到 3 min。注意**不足 60 秒必然显示 0（按分钟取整）**、换不同程序重新计时、浏览器/聊天/游戏不算专注——这是设计不是故障。
> **交付物**：`docs/manual/environment-filesystem-demo-guide.md`——A 环境感知 / B 读侧 / C 写侧 / D 反向边界 / E 常见原因，全部带原话 + 预期 + F12 旁路验证；桌签 shortcut release exe 已重建（13:11，含本轮修复）。验证：lib 528 / all-targets --no-run 0 / release build 0。
> **接手待办**：让用户按上述指南挨个划勾；长期项仍是 25min deep-focus 锁、Settings 切换即时生效的日常手感。`env_probe.py` 等本轮临时探针已删。
> 上一任务（续⁵²·2 dev 真机轮）见下方历史块。
