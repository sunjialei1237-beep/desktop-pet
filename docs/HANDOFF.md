...
> 最后更新：**2026-08-16（续⁴³·拖拽空气墙根治✅：四墙贴视觉身体——头顶可触屏顶、脚底可踩任务栏，气泡随停高位翻转到下巴下方，详见 §当前任务）。上轮 续⁴²（重启问候多元化本地池✅）。上轮 续⁴¹（记忆浮现 LLM 选择器+bubble_log✅）。上轮 续⁴⁰（重启问候单声化✅）。上轮 续³⁹·3（手动拖拽+屏幕墙✅——本轮把墙修到贴身，顶墙出屏放开）。上轮 续³⁹（拖拽落体彻底关闭✅：放哪停哪）。上轮 续³⁶（拖拽剧烈晃动根治✅：GetAsyncKeyState 左键真值门）。上轮 续³⁵（Soul v2 灵魂工程全链路✅，报告 docs/review/soul-upgrade-report-2026-08-15.md）。上轮 续³⁴（二期第一梯队三连✅）。**续⁸ 自主冒泡灵性重构仍在位**（频率 60min + 记忆 15/灵性 85）。**
...
## §当前任务（接手者先看这）
> **2026-08-16（续⁴³）更新 · 拖拽空气墙根治——四墙贴视觉身体，头顶可触屏顶、脚底可踩任务栏 ✅（commit `59fb771` + App.tsx 部分随 `0a54e37` 入库，tsc 0 / vitest 39 / 隔离实例实跑全绿 / release rebuild + 重启）**。用户报告"头顶/脚底像有透明空气墙，拖不到屏幕上下边缘"。**根因两层**：① 顶墙 `minY=0`（窗口顶永不出屏）——璃头顶距窗口顶 240 逻辑px（150px 气泡区 + 90px canvas 空隙），头永远够不到屏幕顶；此墙是续³⁹·2 为躲 OS 原生拖拽松手弹回所设，**但手动拖拽时代（无 startDragging→无系统 move loop）该约束已不存在**（实测坐实，见下）；② 四墙用的是 `onModelBounds` 的 **10% padding 穿透边界框**（PAD+TOP_BIAS），脚底与墙之间有 42 逻辑px 空隙，踩不到任务栏顶（左右各 18px 同理）。**修法**：SpineCanvas 增报无 padding 视觉身体矩形 `onVisualBounds`（visualBoundsRef，__dragDiag 可读）；`clampModelToScreen` 改贴视觉身体——`minY=-mTop`（头顶贴屏顶、窗口顶允许出屏）、`maxY=floor-mBottom`（脚踩任务栏顶）；onMoved 恢复路径补上另两路已有的 `>0.5px 才动` 守卫（y<0 成正常停泊位后防"光标越墙释放"假阳性）。**头顶出屏联动**：气泡锚点 bottom:535/思考球都在头顶上方，窗口顶出屏即整片出屏（她会"哑"）→ PetBubble/thinking-orb 增 `below` 变体：`petPos.y<-6` 时气泡翻转到下巴下方（top:335）向下生长、尾巴 `scaleY(-1)` 镜像朝上，脸仍可见。**验证**（worktree 隔离实例 + CDP 9224 + 新 `scripts/wall_regression.ps1`）：顶墙 T=−360 物理精确（=头顶贴屏顶）、底墙 T=378（脚踩任务栏顶 1368）、右墙 2146、屏中释放原地停留；**顶墙松手零弹回**（lastSnapRestore 恒 null——坐实手动拖拽无 OS 钳制，续³⁹·3 踩坑①的"弹回"是原生拖拽时代观测）；点击头部触发气泡实测 `--below` 类 + 锚点 top=335 + 尾巴元素渲染于气泡顶边（18×13px）。新真实几何：视觉身体窗口内 x∈[124,276] y∈[240,660]，四墙物理 L∈[−186,2146] T∈[−360,378]。**踩坑（本轮新增）**：① **并行会话改 Rust 时 dev watcher 会杀掉测试实例**——拖拽实测用 `git worktree` + `CARGO_TARGET_DIR` 隔离构建 debug exe（HEAD Rust + 拷贝前端改动，注意未跟踪文件如 greetings.ts 不随 worktree 走须手拷）；② **给 parked-high（顶<0）窗口做 UI 自动化别用 Alt+Space**——窗口聚焦时 Alt+Space 打开系统菜单，后续 Enter 触发"还原"把离屏窗口级联回屏（一度误诊为"窗口自己动了"）；触发气泡用 mouse_event 点头部最可靠；③ nohup+& 直接后台启动 exe 会被回收（~20s 退出无日志），用 ZCode 后台任务前台跑。📋 用户日常观察：拖到最上头顶贴屏顶停住、拖到最下脚踩任务栏顶停住、停在最顶时她说话气泡出现在下巴下方。
