...
> **2026-08-18（续⁵²·5 → 续⁵²·6）· VSCode 当前打开文件跨项目定位 + U1 续做路径回喂 ✅（本 commit）**。续⁵²·4 修完后用户仍复测失败：`看看我现在打开的代码/就在我桌面上打开的啊，看不到吗/帮我看一下目前我打开的代码`——真机 capture 坐实最后缺口：VSCode 窗口的 Electron 命令行只保留 STARTUP folder（`D:/environment-demo`），用户在窗口里切去别的项目的 `plan-sess_….md`（真实位置 `D:\桌宠\.zcode\plans\`，标题只有裸文件名），root 解析永远指向启动文件夹；修复后模型把 section root 拼成 `D:/environment-demo/plan-sess_….md`——绝对路径不进入 relative hydration，一直「路径不存在」，授权问自然空转。**本轮修复**：① `window::recent_shortcut_target` 新能力——Windows Recent `*.lnk` 解析：按 `LinkInfo.LocalBasePath` 结构（LinkFlags/VolumeID 偏移）+ `Win32_Globalization::MultiByteToWideChar(CP_ACP)` 解 GBK ANSI 路径（release 本机实测：GBK `D:\桌宠\.zcode\plans\plan-sess_….md` 正确吐出 Utf16 路径）+ 字符扫描兜底；② `environment::repair_absolute_hint`——模型用错误 root 拼出的绝对路径不存在时，按 basename 经 Recent shortcut 换回真实绝对路径，再走统一授权；③ PendingAuthorization/GrantFollowup 新增 `requested_paths`：`可以` 续做时把刚被拦截的真实路径写进系统提示（此前续做轮仍用旧 root 猜 `"/"` 又触发新授权）。**真机复测（release 20:13 构建）**：plan-sess 标签场景 → `absolute hint D:/environment-demo/plan… missing, Recent shortcut target found -> D:\桌宠\.zcode\plans\plan…` → 拒绝时 consent root=`D:\桌宠\.zcode\plans`（不再授权环境演示目录）→ ONCE grant 后下一轮 `read_text_file` 命中真实文件 Success / once-grant 消耗。**文件索引取值边界**：Recent 只作为「文件存在性候选修正」，仍过 fs policy 授权，before grant 模型看不到内容/路径。**同轮再补**：用户 20:34 新开 `raptor-pro.json`，14 秒后问「看看我新打开的代码」仍失败——查 DB 时序，消息发出时新 VSCode 窗口甚至还没建出来，旧窗口样本不可能指向新文件。为让 F12 与下一次提问收敛得更快，observer 端在编辑器 root 缺失、或 root 下没有当前文件时，追加同款 Recent-lnk 反推（root hint 直接指向真实文件的父目录）；20:44 release 真机复测：F12 三秒内显示 `路径: D:\Spine pro 3.8.75+K'D\Spine pro 3.8.75+K'D\成品`；`看看我新打开的代码` → 拒绝同一真实文件 → `可以` → read 成功 + get_file_metadata 成功、once-grant 消费。**接手待办**：让用户按 guide A 段原话验收（注意：先在 VS Code 里把文件打开完、标签页露出来，再问桌宠；采样每 3 秒一次）；`可以` 续做轮的 LLM 速度在上轮被 DeepSeek API 偶发慢响应拖住（无代码错误），如再遇超过 ~60s 无回复是后端请求等待。
