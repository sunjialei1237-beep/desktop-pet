...
> **2026-08-24（续⁵⁶）· 模型配置界面：API key 可见 + 模型方案一键切换 ✅（commit `97cfff9`）**。用户诉求两点：① key 输入永远隐身看不了；② 配置过的模型要能保存、点击切换。实现见文首「最后更新」段。**接手待办**：① 真机验收——打开设置面板确认 key 回显 + 👁 切换、保存一次配置后「已保存的模型」列表出现该方案、配第二个供应商（如 bigmodel）后两案并存、点「使用」切换后 F12/对话即时用新模型；② `llm/client.rs` 的 `thinking_allowed` bigmodel/glm- 分支为并行会话 WIP（未提交），其人继续；③ profile 列表暂无手动重命名/自定义名字（自动以 main_model 命名），需要时再加。
