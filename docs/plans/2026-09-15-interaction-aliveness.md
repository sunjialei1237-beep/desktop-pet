...
**迁移号硬约束（已实测，务必遵守）**：`thought_stream` 表 + 第 8 条迁移记录留在你的库里，而 master 没有 008 文件。master 跑这个库**不报错**——`run_migrations`（`db/schema.rs:5`）开头 `if current_version >= 7 { return Ok(()) }`，`get_schema_version` 取 `MAX(version)`，所以 master 只是静默忽略那张表。**但这引出一条硬约束：不要在 master 上单独加 `008_*.sql`**——活库的 8 已经被分支的 `thought_stream` 占用，master 把守卫改到 8 后会认为"已迁移"而**静默跳过**，新表永不创建。⇒ **`pet_events` 必须是 `009`，且必须先合并 `念头`。**
...
- **落库**：新迁移 **`009_*.sql`**（**不可用 008**——见 §B 末尾的迁移号硬约束；且须先合并 `念头`）+ 上限裁剪 + 插入时 prune；`kind/tag/used_at` 供由头选取与防复用。
