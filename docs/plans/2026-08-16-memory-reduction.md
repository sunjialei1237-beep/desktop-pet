...
> 状态：**P1 ✅（commit `ed5c69d` + `bd61d77`，质量验证通过、fp32 已删）· P2 ✅（懒加载+空闲卸载）· P3 ❌ 已评估否决（2026-08-17 实测，见 §3 P3）**。目标：在不影响正常使用（记忆召回质量、响应速度、动画流畅度）的前提下，把后台内存占用降得越低越好。
> 基线实测总占用 ≈ **1.9 GB**（desktop-pet.exe 1500 MB + 其 WebView2 子进程 429 MB）。P1 后实测总占用 **1318 MB**（desktop-pet.exe 870 MB + WebView2 树 449 MB）。P2 后闲置态主进程 **35-85 MB**（锯齿：对话/调度加载时回到 870MB）。设计文档 §6.11 的性能目标 **内存 < 80 MB** 在闲置段已达成。
...
### P1（✅ 已实施 + 质量验证通过，fp32 已删）：BGE-M3 换成 int8 量化版 ONNX
...
- **✅ 质量验证实测（2026-08-17，`tests/embedding_quality_ab.rs`，fp32 文件删除前跑）**：基准集（18 episodes/12 带答案查询）**fp32 与 int8 top-1 逐条完全一致**（10/12 打平，连答偏的都偏在同条——量化噪声未改变任何决策）；真实库（46 episodes/10 paraphrase 查询）top-5 重合度 **0.90**、8/10 查询 top-1 相同，差异集中在模糊查询腰部排序（"想喝点什么"火锅 vs 雪碧类两义词），且检索里语义分只占 0.4 权重。**结论：无显著质量下降 → 旧 fp32 文件（model.onnx + model.onnx_data，2.16GB）已于同日删除**；harness 在 fp32 缺席时自动跳过。
...
### P3（❌ 已评估并否决，2026-08-17）：换更小的 embedding 模型
- **评估方式**：下载 `bge-small-zh-v1.5` int8（24 MB，512 维）到 `D:\models\bge-small-zh-v1.5`，`tests/embedding_p3_ab.rs` 与 m3-int8 同题对打：基准集（18 episodes/12 带答案查询）hit@1 + 真实库（46 episodes/10 paraphrase 查询）top-5 重合度。门槛（用户钦定）：不影响检索质量。
- **实测**：基准集 hit@1 打平（10/12、semantic 4/6），**但真实库 top-5 重合度仅 0.52（门槛 0.90）**——小模型把「询问昨天有趣的事」这类元对话当成「开心/伤心/喝点什么」的高相关结果（字面碰撞、判别弱），「老家」「项目」排序大幅漂移。标准考题不丢分、真实分布塌方。
- **结论：否决换模型，保留 bge-m3-int8**。用户前提"P3 不影响检索质量"不成立。harness 保留为软报告工具（语料增长或更强小模型出现时可复评）。
- 附带产出：`model.rs` 现支持双输入契约（按 session 图按需喂 `token_type_ids`——bge-*-zh-v1.5 导出必需、BGE-M3 不受影响），换任何 BERT 系导出的集成成本已预付。
- bge-small-zh-v1.5 文件（36 MB）留在 `D:\models` 供未来复评，不碍事。
